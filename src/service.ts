...
export const eligibilityCriteria = (payload: InputCriteria): OutputCriteria => {
  const sumConsumption = payload.historicoDeConsumo.slice(0, MAX_RECENT_VALUES).reduce((acc, num) => acc + num, 0);
  const averageConsumption = sumConsumption / MAX_RECENT_VALUES;
